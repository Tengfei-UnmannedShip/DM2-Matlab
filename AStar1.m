function [SetClose,SetOpen]=AStar1(map,start_row,start_col,end_row,end_col,speed,OSLength,MapSize,drawPath)
%% 根据薛双飞的算法改编
...
%% line1. 设置初始船舶艏向；
background=map;
start_point.row=round((start_row+MapSize(1)*1852)/10); 
start_point.col=round((start_col+MapSize(2)*1852)/10);
destination.row=round((end_row+MapSize(1)*1852)/10);
destination.col=round((end_col+MapSize(2)*1852)/10);
Movelength=round(speed/10);
ShipLong=round(OSLength);
