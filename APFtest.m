...
Boat_Speed_Factor=10;           %速度方向势场衰减因子，取值越大在速度方向上影响越大
BoatRiskFieldPeakValue=100;    %风险最大值可以根据需要随意设置
Boat_eta=0.1;        %改变影响范围，或曲线下降坡度，值越大范围越小
Boat_alfa=0.01;      %改变影响范围，或曲线下降坡度，值越大范围越小
BoatCut=0;
...
if draw==1  %确认绘图
    ...
    hold on;
    plot(ship(3).goalPiont(1),ship(3).goalPiont(2),'ro','MarkerFaceColor','r');
    axis equal;
    axis off;
    figure
    ...
    plot(ship(3).goalPiont(1),ship(3).goalPiont(2),'ro','MarkerFaceColor','r');
    hold on;
    ship_icon(ship(3).pos(1),ship(3).pos(2),ship(3).length,ship(3).width, ship(3).Course,0 ); %这里使用ship_icon函数，显示出来的可能会有点小
    axis equal;
    axis off;
end
